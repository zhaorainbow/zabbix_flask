...
class zabbix_api:
	...
	def ip_get(HostID=''):
		...
		hostip = ip_get(host['hostid'])
                # print host
                if len(hostName) == 0:
                    print "HostID : %s\t %s\t  HostName : %s\t Status :\033[32m%s\033[0m \t Available :\033[31m%s\033[0m" % (
                    host['hostid'], hostip, host['name'], status[host['status']], available[host['available']])
                else:

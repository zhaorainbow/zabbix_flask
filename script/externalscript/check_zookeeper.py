...
class ZooKeeperServer(object):
    ...
    def __init__(self, zkip="localhost", zkport="2181", timeout=1):
	self._zkip = zkip
	self._zkport = zkport
        self._address = (self._zkip, int(self._zkport))
...
#  ps -ef|grep java|grep zookeeper|awk '{print $2}'
         pidarg = '''ps -ef|grep java|grep /usr/local/zookeeper|grep -v grep|awk '{print $2}' '''
...
#         key = "zookeeper.status[" +  metric + "," + "self._zkip" + "," + "self._zkport" + "]"
	 key = "zookeeper.status[%s,%s,%s]" %(metric, self._zkip, self._zkport)
...
if len(sys.argv) >=2 and sys.argv[1] in accepted_modes:
        mode = sys.argv[1]
        if sys.argv[2]:zkip = sys.argv[2]
        if sys.argv[3]:zkport = sys.argv[3]
else:
        usage()
...
zk = ZooKeeperServer(zkip, zkport)
#  print zk.get_stats()
pid = zk.get_pid()

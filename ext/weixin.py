...
######################################
# 可以访问到zabbix页面的URL
URL = 'http://172.28.5.125/zabbix/api_jsonrpc.php'
ZABBIX_URL = 'http://172.28.5.125/zabbix/index.php'
CHART_URL = 'http://172.28.5.125/zabbix/chart.php'
# 登录zabbix的账号密码
USERNAME = 'admin'
PASSWORD = 'zabbix'
# 放cookie的路径
COOKIEURL = '/tmp/cookie'
# 方式图片的路径
PIC_PATH = '/tmp/image/'
...
def getMediaId(ip, key, auth, USERNAME, PASSWORD, COOKIEURL, ZABBIX_URL, PIC_PATH, CHART_URL, corpsecret, corpid):
    ###写入cookie文件
    curlcookie = 'curl  -c %s -b %s -d "name=%s&password=%s&autologin=1&enter=Sign+in" %s ' % (
    COOKIEURL, COOKIEURL, USERNAME, PASSWORD, ZABBIX_URL)
    ...
    curlgraph = 'curl  -b %s -d "itemids=%s&period=3600&time=%s&width=800" %s >%s' % (
    COOKIEURL, itemid, stime, CHART_URL, PIC_URL)
...
if __name__ == '__main__':
    ...
    # 获取图片id
    MEDIA_ID = getMediaId(ip, key, auth, USERNAME, PASSWORD, COOKIEURL, ZABBIX_URL, PIC_PATH, CHART_URL, corpsecret,
                          corpid)
    # 获取token
